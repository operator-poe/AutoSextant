...
public class InputAsync : ExileCore.Input
{
  ...
  private static void IController()
  {
    ...
    _inputController = tryGetInputController("AutoSextant");
    if (_inputController == null)
    {
  ...
  public static async SyncTask<bool> UseItem(Vector2 pos, int delay = 10)
  {
    IController();
    await _inputController.MoveMouse(V2(pos));
    // TODO: adjust cursor class too
  ...
  public static async SyncTask<bool> ClickToInventory(Vector2 pos)
  {
    IController();
    var count = NInventory.Inventory.InventoryCount;
    await _inputController.KeyDown(Keys.ControlKey);
    while (count == NInventory.Inventory.InventoryCount)
    {
      await ClickElement(pos);
      // TODO: maybe actually group those and wait for all of them to be done?
      await Wait();
    }
    await _inputController.KeyUp(Keys.ControlKey);
    IControllerEnd();
    return true;
  }
