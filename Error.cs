...
public class Error
{
    ...
    {
        if (errorList.FindIndex(x => x.Item1 == errorType && x.Item2 == errorMessage) >= 0)
        {
            Show();
            return;
        }
    ...
    {
        showErrorModal = true;
        if (AutoSextant.Instance.IsAnyRoutineRunning)
        {
    ...
    public static void Render()
    {
        if (showErrorModal)
        {
            ImGui.OpenPopup("AutoSextant Alert");
        }
        ...
        if (ImGui.BeginPopupModal("AutoSextant Alert", ref modalOpen, ImGuiWindowFlags.NoResize | ImGuiWindowFlags.NoTitleBar | ImGuiWindowFlags.AlwaysAutoResize))
        {
            ...
            ImGui.TextColored(new System.Numerics.Vector4(1, 1, 1, 1), "AutoSextant Alert");
